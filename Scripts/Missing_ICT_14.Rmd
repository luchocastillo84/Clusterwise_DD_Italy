...
This document outlines the treatment of missing data before applying dimensionality reduction using Principal Component Analysis PCA  for the `params$data_year` data frame, which represents the ICT usage in Italian enterprises survey for the year 2014.
...
```{r cloud benefits by region, echo=FALSE, fig.height=13, fig.width=13}
# Plot with facets by company region
ggplot(long_data1a, aes(x = dom1, fill = categorized)) +
  geom_bar(position = "stack") +
...
```{r auto-process invoices by region, echo=FALSE, fig.height=13, fig.width=13}
# Plot with facets by company Region
long_data2 %>% filter(variable == "I2a") %>%  ggplot(aes(x = dom1, fill = categorized)) +
  geom_bar(position = "stack") +
  labs(title = "Distribution of Auto-Process Invoices by Sector and size", 
...
```{r Manual e-invoices by region, echo=FALSE, fig.height=13, fig.width=13}
...
```{r paper invoices by region, echo=FALSE, fig.height=13, fig.width=13}
# Plot with facets by company Region
long_data2 %>% filter(variable == "I2c") %>%  ggplot(aes(x = dom1, fill = categorized)) +
  geom_bar(position = "stack") +
